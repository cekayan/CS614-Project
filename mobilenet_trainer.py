...
class MobileNetV2Classifier(nn.Module):
    """
    MobileNetV2 model with advanced classifier for 3-class classification.
    ...
    def __init__(self, num_classes=3, freeze_features=False):
        ...
        # Load pre-trained MobileNetV2 (using V2 weights like optimized trainer)
...
def main():
    """
    Main training pipeline for MobileNetV2.
    """
    ...
    BATCH_SIZE = 32  # MobileNet can handle larger batches due to lower memory usage
    ...
    model = MobileNetV2Classifier(num_classes=len(class_names), freeze_features=False)
    ...
    # Using transfer learning approach - separate learning rates
    # Separate parameters into feature extractor and classifier
    feature_params = []
    classifier_params = list(model.mobilenet.classifier.parameters())
    
    # Get all feature parameters (everything except classifier)
    for name, param in model.mobilenet.named_parameters():
        if not name.startswith('classifier.'):
            feature_params.append(param)
    
    # MobileNet typically works well with slightly higher learning rates
    optimizer = optim.Adam([
        {'params': classifier_params, 'lr': 2e-3},  # Higher LR for new classifier
        {'params': feature_params, 'lr': 5e-4}     # Moderate LR for pre-trained features
    ])
    
    # Learning rate scheduler - more aggressive for mobile nets
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5)
    
    # Training
    print("\nStarting training...")
    NUM_EPOCHS = 20  # MobileNet often needs a few more epochs
    ...
    torch.save(model.state_dict(), './mobilenetv2_osteosarcoma.pth')
    print("Model saved as 'mobilenetv2_osteosarcoma.pth'")
