...
class DenseNetClassifier(nn.Module):
    """
    DenseNet121 model with advanced classifier for 3-class classification.
...
def main():
    """
    Main training pipeline for DenseNet121.
    """
    print("Starting DenseNet121 Transfer Learning Pipeline")
    print("=" * 60)
    ...
    BATCH_SIZE = 16  # DenseNet can be memory intensive
    ...
    model = DenseNetClassifier(num_classes=len(class_names), freeze_features=False)
    ...
    feature_params = []
    classifier_params = list(model.densenet.classifier.parameters())
    ...
    # DenseNet works well with moderate learning rates
    ...
    # Learning rate scheduler - DenseNet benefits from gradual reduction
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=6, gamma=0.2)
    
    # Training
    print("\nStarting training...")
    NUM_EPOCHS = 18  # DenseNet often needs slightly more epochs
    ...
    torch.save(model.state_dict(), './densenet121_osteosarcoma.pth')
    print("Model saved as 'densenet121_osteosarcoma.pth'")
